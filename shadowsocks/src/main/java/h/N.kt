...
class N : BroadcastReceiver() {
    companion object {
        private val componentName by lazy { ComponentName(app, N::class.java) }
        var enabled: Boolean
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val doStart = when (intent.action) {
            ...
            else -> DataStore.directBootAware || Build.VERSION.SDK_INT >= 24 && V.user.isUserUnlocked
        }
        if (doStart) V.startService()
    }
}
