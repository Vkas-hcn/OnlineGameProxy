...
class M : Service(), CoroutineScope {
    companion object {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (worker == null) {
            ...
            worker = launch {
                val urls = BE.instance.urls
                val notification = NotificationCompat.Builder(this@M, NOTIFICATION_CHANNEL).apply {
                    color = ContextCompat.getColor(this@M, R.color.material_primary_500)
                    priority = NotificationCompat.PRIORITY_LOW
                    addAction(NotificationCompat.Action.Builder(
                            R.drawable.ic_navigation_close,
                            getText(R.string.stop),
                            PendingIntent.getBroadcast(this@M, 0,
                ...
                try {
                    ...
                    GlobalScope.launch(Dispatchers.Main) {
                        V.notification.cancel(NOTIFICATION_ID)
                        idle.value = true
                    }
                    check(worker != null)
                    worker = null
    ...
    private fun fetchJsonAsync(url: URL, max: Int, notification: NotificationCompat.Builder) = async(Dispatchers.IO) {
        ...
        try {
            ...
            launch(Dispatchers.Main) {
                Toast.makeText(this@M, e.readableMessage, Toast.LENGTH_LONG).show()
            }
