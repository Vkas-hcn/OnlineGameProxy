...
object Z {
    enum class State(val canStop: Boolean = false) {
        /**
         * Idle state is only used by UI and will never be returned by Z.
         */
        Idle,
        Connecting(true),
        Connected(true),
    ...
    class Data internal constructor(private val service: Interface) {
        ...
        var processes: BC? = null
        var proxy: X? = null
        var udpFallback: X? = null
        var localDns: BB? = null

        var notification: BD? = null
    ...
    interface Interface {
        val data: Data
        val tag: String
        fun createNotification(profileName: String): BD
        ...
        suspend fun startProcesses() {
            val context = if (Build.VERSION.SDK_INT < 24 || V.user.isUserUnlocked) app else V.deviceStorage
            val configRoot = context.noBackupFilesDir
            val udpFallback = data.udpFallback
            data.proxy!!.start(this,
                    File(V.deviceStorage.noBackupFilesDir, "stat_main"),
            ...
            udpFallback?.start(this,
                    File(V.deviceStorage.noBackupFilesDir, "stat_udp"),
                    File(configRoot, CONFIG_FILE_UDP),
                    "udp_only", false)
            data.localDns = BB(this::rawResolver).apply { start() }
        }
...
//                Firebase.analytics.logEvent("stop") { param(FirebaseAnalytics.Param.METHOD, tag) }
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            val profile = V.currentProfile
            Log.i(Z::class.simpleName, "onStartCommand: use profile[id = ${profile?.id}], host = ${profile?.host}, name = ${profile?.name}")
            this as Context
            if (profile == null) {
                Log.i(Z::class.simpleName, "onStartCommand: no available profile, stop runner automatically")
            ...
            try {
                data.proxy = X(profile)
                data.udpFallback = null //if (fallback == null) null else X(fallback, profile.route)
            ...
            N.enabled = DataStore.persistAcrossReboot
            if (!data.closeReceiverRegistered) {
...
//            Firebase.analytics.logEvent("start") { param(FirebaseAnalytics.Param.METHOD, tag) }
            data.connectingJob = GlobalScope.launch(Dispatchers.Main) {
                try {
                    W.killAll()    // clean up old processes
                    preInit()
