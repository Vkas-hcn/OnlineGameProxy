 ...
 */
class X(val profile: Profile, private val route: String = profile.route) {
    init {
...
//        }
     ...
     */
    fun start(service: Z.Interface, stat: File, configFile: File, mode: String, dnsRelay: Boolean = true) {
        // setup traffic monitor path
        trafficMonitor = Y(stat)
        ...
        config.put("locals", JSONArray().apply {
            ...
            if (dnsRelay) try {
                ...
                throw Z.ExpectedExceptionWrapper(e)
            }.let { dns ->
                put(JSONObject().apply {
                    put("local_address", DataStore.listenAddress)
        ...
        val cmd = arrayListOf(
                File((service as Context).applicationInfo.nativeLibraryDir, W.SS_LOCAL).absolutePath,
