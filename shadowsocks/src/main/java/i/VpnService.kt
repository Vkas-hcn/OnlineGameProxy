...
class VpnService : BaseVpnService(), Z.Interface {
    companion object {
    ...
    private inner class ProtectWorker : ConcurrentLocalSocketListener("ShadowsocksVpnThread",
        File(V.deviceStorage.noBackupFilesDir, "protect_path")) {
...
//                            OsConstants.EPERM, OsConstants.EACCES, OsConstants.ENONET -> Timber.d(e)
    ...
    inner class NullConnectionException : NullPointerException(), Z.ExpectedException {
        override fun getLocalizedMessage() = getString(R.string.reboot_required)
    }

    override val data = Z.Data(this)
    override val tag: String get() = "ShadowsocksVpnService"
    override fun createNotification(profileName: String): BD =
        BD(this, profileName, "service-vpn")
    ...
    override fun onBind(intent: Intent) = when (intent.action) {
        ...
        else -> super<Z.Interface>.onBind(intent)
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (DataStore.serviceMode == Key.modeVpn) {
            if (prepare(this) != null) {
                startActivity(Intent(this, I::class.java).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
            } else return super<Z.Interface>.onStartCommand(intent, flags, startId)
        }
        stopRunner()
    ...
    private suspend fun startVpn(): FileDescriptor {
        ...
        val builder = Builder()
//                .setConfigureIntent(V.configureIntent(this))
        ...
        BA.brand(this, builder, packageName)
...
//        }
        ...
        val cmd = arrayListOf(File(applicationInfo.nativeLibraryDir, W.TUN2SOCKS).absolutePath,
    ...
    private suspend fun sendFd(fd: FileDescriptor) {
        var tries = 0
        val path = File(V.deviceStorage.noBackupFilesDir, "sock_path").absolutePath
        while (true) try {
            delay(50L shl tries)
