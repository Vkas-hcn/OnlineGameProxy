...
class E : BaseActivityNew(),
    ...
    companion object {
        var stateListener: ((Z.State) -> Unit)? = null
    }
    ...
    fun apolloPlugOgAdvertisementShow(it: Boolean) {
        AdBase.getConnectInstance().advertisementLoadingOg(this@E)
    ...
    override fun initData() {
        ...
        if (model.whetherParsingIsIllegalIp()) {
            model.whetherTheBulletBoxCannotBeUsed(this@E)
            return
        }
        ...
        // 设置状态
        changeState(Z.State.Idle, animate = false)
        ...
        liveJumpResultsPage.observe(this, {
            lifecycleScope.launch(Dispatchers.Main.immediate) {
                ...
                if (lifecycle.currentState == Lifecycle.State.RESUMED) {
                    val intent = Intent(this@E, G::class.java)
                    intent.putExtras(it)
...
//                val showState =
//                    OgLoadConnectAd
//                        .displayConnectAdvertisementOg(this@E)
//                if (showState != 2) {
...
//            }
//            AdBase.getConnectInstance().advertisementLoadingOg(this@E)
////            AdBase.getResultInstance().advertisementLoadingOg(this@E)
//            try {
//                withTimeout(10000L) {
//                    delay(2000L)
    ...
    private fun startVpn() {
        ...
        jobStartOg = lifecycleScope.launch {
            ...
            if (isThresholdReached || idOgEmpty) {
                model.handleThresholdOrEmptyIdOg(this@E)
                return@launch
            }
            ...
            try {
                withTimeout(10000L) {
                    ...
                    while (jobStartOg?.isActive == true) {
                        val showState =
                            OgLoadConnectAd.displayConnectAdvertisementOg(this@E)
                        ...
                        when (showState) {
                            ...
                            "00" -> {
                                jobStartOg?.cancel()
                                jobStartOg = null
                                model.connectOrDisconnectOg(false,this@E)
                            }
                        }
...
//            EasyConnectUtils.getBuriedPointOg("unlimF_vF")
    ...
    private fun connectionServerSuccessful() {
        vpnStateFun(2)
        changeOfVpnStatus()
        C.isVpnGlobalLink = true
        S.getHeartbeatReportedConnect(C.isVpnGlobalLink, this)
    }
    ...
    private fun disconnectServerSuccessful() {
        vpnStateFun(0)
        changeOfVpnStatus()
        C.isVpnGlobalLink = false
        S.getHeartbeatReportedDisConnect()
    }
    ...
    override fun stateChanged(state: Z.State, profileName: String?, msg: String?) {
        changeState(state)
    }
        ...
        lifecycleScope.launch {
            ...
            if (C.nativeAdRefreshOg) {
                ...
                if (AdBase.getHomeInstance().appAdDataOg != null) {
                    ...
                    AdBase.getHomeInstance().advertisementLoadingOg(this@E)
                    initHomeAd()
                }
            }
        }
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (viewGuideMask.isVisible) {
                ...
                if (!(model.state.name != "Connected" && S.vpnState == 1)) {
                    finish()
                }
            }
        }
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.tv_privacy_policy_pt -> {
                val intent = Intent(this@E, H::class.java)
